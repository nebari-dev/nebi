...
- **`GET /remote/server` reporting `status: 'connected'` means a server URL + token are stored** (a local DB read), not that the remote is reachable. Reachability surfaces as errors on the remote data queries; pages gate remote data and the unreachable banner with `useRemoteView()` from `src/hooks/useRemote.ts`, and consume the `isFirstLoad` / `isUnreachable` flags the remote query hooks return rather than re-deriving them from TanStack internals. Any query whose `isUnreachable` flag a page renders must keep retrying on its own — `pollWithErrorBackoff` if it polls anyway, `retryWhileUnreachable` if it shouldn't poll while healthy — because an errored query without an interval only refetches on remount or window focus, so the banner would stick after the server recovered. The remote workspace *detail* page (`RemoteWorkspaceDetail.tsx`) and the per-workspace hooks predate this pattern and are not yet migrated (tracked in https://github.com/nebari-dev/nebi/issues/507). If that status ever becomes a real liveness probe, revisit the banner logic, which assumes it never flips on remote outages.
