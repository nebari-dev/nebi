...
- **`GET /remote/server` reporting `status: 'connected'` means a server URL + token are stored** (a local DB read), not that the remote is reachable. Reachability surfaces as errors on the remote data queries; `useRemoteView()` in `src/hooks/useRemote.ts` is the shared derivation pages should use to gate remote data and the unreachable banner. If that status ever becomes a real liveness probe, revisit the banner logic, which assumes it never flips on remote outages.
