...
func TestProjectDirMissing(t *testing.T) {
	tests := []struct {
		name  string
		files []string // files to create inside the project dir
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := projectDirMissing(dir); got != tt.want {
				t.Errorf("projectDirMissing(%q) = %v, want %v", dir, got, tt.want)
			}
		})
	}
}
