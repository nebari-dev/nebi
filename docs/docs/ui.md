...
Admins can review blocked external identity links from **Admin -> Identity Reviews**. A review appears when an OIDC, proxy-auth, or device-flow login presents an issuer/subject pair that is not yet bound to a Nebi user but its username or email claim collides with an existing account.

Each row shows the target user, issuer, subject, collision field, profile claims, status, and review time. **Approve** permanently binds that issuer/subject to the shown account and future logins use that binding even if username or email claims later change. **Reject** blocks that issuer/subject from linking to the account; there is currently no undo action in the UI.
