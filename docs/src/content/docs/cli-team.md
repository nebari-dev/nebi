...
If you don't have a server for your team or organization yet, see [Server Setup](/server-setup/).
...
Before publishing, you need to configure an OCI registry with credentials. See [Registry Setup](/registry-setup/) for step-by-step instructions on setting up GHCR or Quay.io.
...
The OCI manifest document is capped at 1 MiB, so extremely large bundle file lists fail before upload.
