...
describe('UseLocallyButton', () => {
  it('opens the local usage panel and copies the pull command', async () => {
    ...
    const command = `nebi login ${window.location.origin} && nebi pull ml-project`;

    renderWithProviders(<UseLocallyButton projectName="ml-project" />);

    await user.click(screen.getByRole('button', { name: /use locally/i }));

    expect(
      screen.getByRole('dialog', { name: /use this project locally/i }),
    ).toBeInTheDocument();
