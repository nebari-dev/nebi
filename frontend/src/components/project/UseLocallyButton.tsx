...
export const UseLocallyButton = ({ projectName }: UseLocallyButtonProps) => {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
  const panelId = useId();
  const titleId = useId();
  const command = `nebi login ${window.location.origin} && nebi pull ${projectName}`;
        ...
        >
          <div className="space-y-3 p-4 pr-10">
            ...
            <p className="text-sm leading-6 text-muted-foreground">
              Run this in your terminal to download the project spec to your
