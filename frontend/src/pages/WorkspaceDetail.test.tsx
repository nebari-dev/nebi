...
import { mockJob, mockUser, mockWorkspace, server } from '@/test/handlers';
import { renderWithProviders } from '@/test/utils';
import { WorkspaceDetail } from './WorkspaceDetail';

const manifest = '[workspace]\nname = "test-workspace"\n';

function renderWorkspace(canWrite?: boolean) {
  server.use(
    http.get('/api/v1/workspaces/ws-1', () =>
      HttpResponse.json({
        ...mockWorkspace,
  ...
  return renderWithProviders(
    ...
    { initialEntries: ['/workspaces/ws-1'] },
  );
}
...
beforeEach(() => {
  ...
  server.use(
    http.get('/api/v1/workspaces/ws-1/packages', () => HttpResponse.json([])),
    http.get('/api/v1/workspaces/ws-1/pixi-toml', () =>
...
describe('Workspace configuration permissions', () => {
  it.each([false, undefined])(
    'keeps configuration readable and both Edit buttons disabled when can_write is %s',
    async (canWrite) => {
      const save = vi.spyOn(workspacesApi, 'savePixiToml');
      const solve = vi.spyOn(workspacesApi, 'solveWorkspace');
      const user = userEvent.setup();
      renderWorkspace(canWrite);
  ...
  it.each(['header', 'configuration tab'])(
    ...
    async (entryPoint) => {
      const save = vi.spyOn(workspacesApi, 'savePixiToml').mockResolvedValue();
      const solve = vi
        .spyOn(workspacesApi, 'solveWorkspace')
