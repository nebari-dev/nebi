...
// ProjectVersion represents a snapshot of project files at a point in time
type ProjectVersion struct {
	...
	// Version tracking
	VersionNumber int `gorm:"not null;index:idx_project_version" json:"version_number"` // Auto-incrementing per project
...
func (wv *ProjectVersion) BeforeCreate(tx *gorm.DB) error {
	if wv.ID == uuid.Nil {
		wv.ID = uuid.New()
	}
