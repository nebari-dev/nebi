package service

import (
	"context"
	"os"
	"path/filepath"
	"strings"
	"testing"

	"github.com/nebari-dev/nebi/internal/audit"
	"github.com/nebari-dev/nebi/internal/limits"
	"github.com/nebari-dev/nebi/internal/models"
)

// --- InstallProjectEnv tests ---

func TestInstallProjectEnv_CreatesJob(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "env-install", userID)

	job, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if job.Type != models.JobTypeEnvInstall {
		t.Errorf("expected job type %q, got %q", models.JobTypeEnvInstall, job.Type)
	}
	if job.Status != models.JobStatusPending {
		t.Errorf("expected job status %q, got %q", models.JobStatusPending, job.Status)
	}
	if job.ProjectID != project.ID {
		t.Errorf("expected project ID %s, got %s", project.ID, job.ProjectID)
	}
}

func TestInstallProjectEnv_RejectedInTeamMode(t *testing.T) {
	svc, db := testSetup(t, false)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "team-install", userID)

	_, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID)
	var ve *ValidationError
	if !isValidationError(err, &ve) {
		t.Fatalf("expected ValidationError in team mode, got %T: %v", err, err)
	}
}

func TestInstallProjectEnv_RejectsWhileEnvJobActive(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "double-install", userID)

	if _, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID); err != nil {
		t.Fatalf("first install: %v", err)
	}

	_, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID)
	if err == nil {
		t.Fatal("expected second install to be rejected while first job is pending")
	}
}

func TestInstallProjectEnv_RejectsOversizedLockBeforeJobWrite(t *testing.T) {
	svc, db := testSetup(t, true)
	limitCfg := limits.Defaults()
	limitCfg.LockBytes = 8
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "env-lock-limit", userID)
	if err := writeProjectFiles(t, svc, project, "[project]\nname = \"env-lock-limit\"\n", strings.Repeat("x", 9)); err != nil {
		t.Fatal(err)
	}

	_, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID)

	var ve *ValidationError
	if !isValidationError(err, &ve) {
		t.Fatalf("expected ValidationError, got %T: %v", err, err)
	}
	var jobs int64
	db.Model(&models.Job{}).Where("project_id = ? AND type = ?", project.ID, models.JobTypeEnvInstall).Count(&jobs)
	if jobs != 0 {
		t.Fatalf("expected no env-install job writes, got %d", jobs)
	}
	var auditCount int64
	db.Model(&models.AuditLog{}).Where("user_id = ? AND action = ?", userID, audit.ActionInstallEnv).Count(&auditCount)
	if auditCount != 0 {
		t.Fatalf("expected no env-install audit writes, got %d", auditCount)
	}
}

// --- install_status derivation tests ---

func TestGet_InstallStatus_NotInstalled(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-notinstalled", userID)

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != models.InstallStatusNotInstalled {
		t.Errorf("expected install_status %q, got %q", models.InstallStatusNotInstalled, resp.InstallStatus)
	}
}

func TestGet_InstallStatus_InstalledFromDisk(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-installed", userID)
	makeEnvInstalled(t, svc, project)

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != models.InstallStatusInstalled {
		t.Errorf("expected install_status %q, got %q", models.InstallStatusInstalled, resp.InstallStatus)
	}
}

func TestGet_InstallStatus_InstallingWhileJobActive(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-installing", userID)

	if _, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID); err != nil {
		t.Fatalf("install: %v", err)
	}

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != models.InstallStatusInstalling {
		t.Errorf("expected install_status %q, got %q", models.InstallStatusInstalling, resp.InstallStatus)
	}
}

func TestGet_InstallStatus_UninstallingWhileJobActive(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-uninstalling", userID)
	makeEnvInstalled(t, svc, project)

	if _, err := svc.UninstallProjectEnv(context.Background(), project.ID.String(), userID); err != nil {
		t.Fatalf("uninstall: %v", err)
	}

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != models.InstallStatusUninstalling {
		t.Errorf("expected install_status %q, got %q", models.InstallStatusUninstalling, resp.InstallStatus)
	}
}

func TestGet_InstallStatus_FailedWhenLastInstallJobFailed(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-failed", userID)

	job, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID)
	if err != nil {
		t.Fatalf("install: %v", err)
	}
	if err := db.Model(job).Update("status", models.JobStatusFailed).Error; err != nil {
		t.Fatalf("fail job: %v", err)
	}

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != models.InstallStatusFailed {
		t.Errorf("expected install_status %q, got %q", models.InstallStatusFailed, resp.InstallStatus)
	}
}

// TestGet_InstallStatus_FailedWinsOverStaleDir proves a failed install job
// is reported even when it left a partial .pixi/envs directory behind:
// pixi install can create the dir before failing partway through, and
// dir-existence alone must not be read as a successful install.
func TestGet_InstallStatus_FailedWinsOverStaleDir(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-failed-stale-dir", userID)
	makeEnvInstalled(t, svc, project)

	job, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID)
	if err != nil {
		t.Fatalf("install: %v", err)
	}
	if err := db.Model(job).Update("status", models.JobStatusFailed).Error; err != nil {
		t.Fatalf("fail job: %v", err)
	}

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != models.InstallStatusFailed {
		t.Errorf("expected install_status %q, got %q", models.InstallStatusFailed, resp.InstallStatus)
	}
}

func TestGet_InstallStatus_OmittedInTeamMode(t *testing.T) {
	svc, db := testSetup(t, false)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "st-team", userID)

	resp, err := svc.Get(project.ID.String(), project.OwnerID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if resp.InstallStatus != "" {
		t.Errorf("expected empty install_status in team mode, got %q", resp.InstallStatus)
	}
}

func TestList_InstallStatusPopulatedInLocalMode(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	installed := createReadyProject(t, svc, db, "list-installed", userID)
	makeEnvInstalled(t, svc, installed)
	createReadyProject(t, svc, db, "list-plain", userID)

	list, err := svc.List(userID)
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	got := map[string]models.InstallStatus{}
	for _, r := range list {
		got[r.Name] = r.InstallStatus
	}
	if got["list-installed"] != models.InstallStatusInstalled {
		t.Errorf("expected list-installed=installed, got %q", got["list-installed"])
	}
	if got["list-plain"] != models.InstallStatusNotInstalled {
		t.Errorf("expected list-plain=not_installed, got %q", got["list-plain"])
	}
}

// makeEnvInstalled creates the on-disk .pixi/envs marker for a project.
func makeEnvInstalled(t *testing.T, svc *ProjectService, project *models.Project) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(svc.GetProjectPath(project), ".pixi", "envs"), 0o755); err != nil {
		t.Fatal(err)
	}
}

// --- UninstallProjectEnv tests ---

func TestUninstallProjectEnv_CreatesJob(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "env-uninstall", userID)

	job, err := svc.UninstallProjectEnv(context.Background(), project.ID.String(), userID)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if job.Type != models.JobTypeEnvUninstall {
		t.Errorf("expected job type %q, got %q", models.JobTypeEnvUninstall, job.Type)
	}
}

func TestUninstallProjectEnv_RejectsWhileEnvJobActive(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "double-uninstall", userID)

	if _, err := svc.InstallProjectEnv(context.Background(), project.ID.String(), userID); err != nil {
		t.Fatalf("install: %v", err)
	}

	_, err := svc.UninstallProjectEnv(context.Background(), project.ID.String(), userID)
	if err == nil {
		t.Fatal("expected uninstall to be rejected while install job is pending")
	}
}
