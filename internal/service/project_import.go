...
// creates a new project populated from it.
//
...
// a project-create job that points the worker at the staging dir
// (CreateProjectOptions.SeedDir). The mode difference is purely how
// many bytes get staged:
//
//   - local mode: oci.ExtractBundle streams pixi.toml + pixi.lock + every
//     asset layer to disk. The worker seeds the project from the full
...
func (s *ProjectService) ImportFromRegistry(ctx context.Context, registryID string, req ImportFromRegistryRequest, userID uuid.UUID) (*models.Project, error) {
	...
	project, err := s.Create(ctx, CreateRequest{
		Name:             req.Name,
	...
	audit.LogAction(s.db, userID, audit.ActionImportProject, fmt.Sprintf("project:%s", project.ID.String()), map[string]interface{}{
		"name":       req.Name,
