...
`), 0o644)
	...
	project, err := svc.ImportFromRegistry(context.Background(), dbReg.ID.String(), ImportFromRegistryRequest{
	...
	if project.Name != "imported-env" {
		t.Errorf("project name: got %q want %q", project.Name, "imported-env")
	}
	...
	if err := db.Where("user_id = ? AND action = ?", userID, "import_project").Find(&audits).Error; err != nil {
		t.Fatalf("query audit logs: %v", err)
	}
	if len(audits) != 1 {
		t.Fatalf("expected exactly one import_project audit row, got %d", len(audits))
	}
...
func TestImportFromRegistry_RequiresRegistryReadAccess(t *testing.T) {
	...
	var count int64
	db.Model(&models.Project{}).Where("name = ?", "imported-env").Count(&count)
	if count != 0 {
		t.Fatalf("expected no project to be created without registry read grant, got %d", count)
	}
}
