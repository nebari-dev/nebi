...
func TestInstallPackages_CreatesJob(t *testing.T) {
	...
	project := createReadyProject(t, svc, db, "install-test", userID)

	job, err := svc.InstallPackages(context.Background(), project.ID.String(), []string{"numpy", "pandas"}, userID)
	if err != nil {
	...
	if job.ProjectID != project.ID {
		t.Errorf("expected project ID %s, got %s", project.ID, job.ProjectID)
	}
...
func TestInstallPackages_RejectsNotReady(t *testing.T) {
	...
	// Create project but don't mark ready (stays pending)
	project, _ := svc.Create(context.Background(), CreateRequest{Name: "pending"}, userID)

	_, err := svc.InstallPackages(context.Background(), project.ID.String(), []string{"numpy"}, userID)
	if err == nil {
		t.Fatal("expected error for non-ready project")
	}
...
func TestInstallPackages_AllowsLargePackageRequest(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "many-pkgs", userID)
	...
	if _, err := svc.InstallPackages(context.Background(), project.ID.String(), packages, userID); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	var jobs int64
	db.Model(&models.Job{}).Where("project_id = ? AND type = ?", project.ID, models.JobTypeInstall).Count(&jobs)
	if jobs != 1 {
		t.Fatalf("expected 1 install job, got %d", jobs)
	}
}

func TestInstallPackages_RejectsOversizedProjectManifestBeforeJobWrite(t *testing.T) {
	svc, db := testSetup(t, true)
	limitCfg := limits.Defaults()
	limitCfg.ManifestBytes = 8
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "install-manifest-limit", userID)
	if err := writeProjectFiles(t, svc, project, strings.Repeat("x", 9), "version: 6\n"); err != nil {
		t.Fatal(err)
	}
...
func TestInstallPackages_RejectsPerUserQuotaBeforeJobWrite(t *testing.T) {
	...
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "user-quota", userID)
...
func TestInstallPackages_RejectsPerProjectQuotaBeforeJobWrite(t *testing.T) {
	svc, db := testSetup(t, true)
	limitCfg := limits.Defaults()
	limitCfg.ActiveJobsPerUser = 10
	limitCfg.ActiveJobsPerProject = 1
	limitCfg.ActiveJobsGlobal = 10
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "project-quota", userID)
	if err := db.Model(&models.Job{}).Where("project_id = ?", project.ID).Update("status", models.JobStatusCompleted).Error; err != nil {
	...
	db.Model(&models.Job{}).Where("project_id = ? AND type = ?", project.ID, models.JobTypeInstall).Count(&installJobs)
	if installJobs != 0 {
	...
	if snapshot.QuotaRejections.Project != 1 {
		t.Fatalf("expected durable project quota rejection metric, got %+v", snapshot.QuotaRejections)
	}
}
...
func TestInstallPackages_RejectsGlobalQuotaBeforeJobWrite(t *testing.T) {
	...
	limitCfg := limits.Defaults()
	limitCfg.ActiveJobsPerUser = 10
	limitCfg.ActiveJobsPerProject = 10
	limitCfg.ActiveJobsGlobal = 1
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "global-quota", userID)
	if err := db.Model(&models.Job{}).Where("project_id = ?", project.ID).Update("status", models.JobStatusCompleted).Error; err != nil {
		t.Fatalf("complete setup job: %v", err)
	}
	if err := db.Create(&models.Job{
...
func TestInstallPackages_LegacyOwnerJobCountsAgainstPerUserQuota(t *testing.T) {
	...
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "legacy-user-quota", userID)
...
func TestInstallPackages_ConcurrentPerUserQuotaBeforeJobWrite(t *testing.T) {
	...
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "concurrent-user-quota", userID)
	if err := db.Model(&models.Job{}).Where("project_id = ?", project.ID).Update("status", models.JobStatusCompleted).Error; err != nil {
		t.Fatalf("complete setup job: %v", err)
	}
	...
	for i := 0; i < attempts; i++ {
		...
		go func(i int) {
			defer wg.Done()
			<-start
			_, err := svc.InstallPackages(context.Background(), project.ID.String(), []string{fmt.Sprintf("pkg-%d", i)}, userID)
			errs <- err
		}(i)
	}
	close(start)
	...
	db.Model(&models.Job{}).
		Where("project_id = ? AND type = ? AND status IN ?", project.ID, models.JobTypeInstall, activeJobStatuses).
...
func TestInstallPackages_RejectsOversizedMetadataBeforeJobWrite(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "metadata-limit", userID)
	if err := db.Model(&models.Job{}).Where("project_id = ?", project.ID).Update("status", models.JobStatusCompleted).Error; err != nil {
		t.Fatalf("complete setup job: %v", err)
	}
	limitCfg := limits.Defaults()
...
func TestSolveProject_RejectsOversizedProjectManifestBeforeJobWrite(t *testing.T) {
	svc, db := testSetup(t, true)
	limitCfg := limits.Defaults()
	limitCfg.ManifestBytes = 8
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "solve-manifest-limit", userID)
	if err := writeProjectFiles(t, svc, project, strings.Repeat("x", 9), "version: 6\n"); err != nil {
		t.Fatal(err)
	}

	_, err := svc.SolveProject(context.Background(), project.ID.String(), userID)
	...
	db.Model(&models.Job{}).Where("project_id = ? AND type = ?", project.ID, models.JobTypeUpdate).Count(&updateJobs)
	if updateJobs != 0 {
...
func TestRemovePackage_CreatesJob(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "remove-test", userID)

	job, err := svc.RemovePackage(context.Background(), project.ID.String(), "numpy", userID)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestRemovePackage_RejectsNotReady(t *testing.T) {
	...
	_, err := svc.RemovePackage(context.Background(), project.ID.String(), "numpy", userID)
	var ve *ValidationError
	if !isValidationError(err, &ve) {
		t.Fatalf("expected ValidationError, got %T: %v", err, err)
	}
...
func writeProjectFiles(t *testing.T, svc *ProjectService, project *models.Project, manifest string, lock string) error {
	t.Helper()
	projectPath := svc.executor.GetProjectPath(project)
	if err := os.MkdirAll(projectPath, 0o755); err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(projectPath, "pixi.toml"), []byte(manifest), 0o644); err != nil {
		return err
	}
	return os.WriteFile(filepath.Join(projectPath, "pixi.lock"), []byte(lock), 0o644)
}
...
func TestListPackages_Empty(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "empty-pkgs", userID)

	pkgs, err := svc.ListPackages(project.ID.String())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(pkgs) != 0 {
...
func TestListPackages_ReturnsInserted(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "with-pkgs", userID)

	// Simulate worker having saved packages
	db.Create(&models.Package{ProjectID: project.ID, Name: "numpy", Version: "1.24.0"})
	db.Create(&models.Package{ProjectID: project.ID, Name: "pandas", Version: "2.0.0"})
...
func TestSyncPackagesFromProject_SavesAllResolvedPackages(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "listed-package-limit", userID)

	stubPixiList(t, []pixi.Package{
		{Name: "numpy", Version: "1.0.0"},
		{Name: "pandas", Version: "2.0.0"},
	})
	if err := svc.SyncPackagesFromProject(context.Background(), project); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	var packages []models.Package
	if err := db.Where("project_id = ?", project.ID).Find(&packages).Error; err != nil {
...
func TestSaveInstalledPackages(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "save-pkgs", userID)

	svc.SaveInstalledPackages(project.ID, []string{"scipy", "matplotlib"})

	var count int64
	db.Model(&models.Package{}).Where("project_id = ?", project.ID).Count(&count)
	if count != 2 {
...
func TestDeletePackagesByName(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "del-pkgs", userID)

	db.Create(&models.Package{ProjectID: project.ID, Name: "numpy"})
	db.Create(&models.Package{ProjectID: project.ID, Name: "pandas"})
	db.Create(&models.Package{ProjectID: project.ID, Name: "scipy"})

	svc.DeletePackagesByName(project.ID, []string{"numpy", "pandas"})
...
func TestDeleteAllPackages(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "del-all", userID)

	db.Create(&models.Package{ProjectID: project.ID, Name: "numpy"})
	db.Create(&models.Package{ProjectID: project.ID, Name: "pandas"})

	svc.DeleteAllPackages(project.ID)
...
// solve must not leave the project terminally stuck. A project in the
// "failed" state must accept a new solve job so a corrected spec can recover it.
func TestSolveProject_AllowedOnFailedProject(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "solve-after-failure", userID)
	if err := writeProjectFiles(t, svc, project, "[project]\nname = \"solve-after-failure\"\n", "version: 6\n"); err != nil {
		t.Fatal(err)
	}
	db.Model(project).Update("status", models.ProjectStatusFailed)

	job, err := svc.SolveProject(context.Background(), project.ID.String(), userID)
	if err != nil {
		t.Fatalf("solve on failed project should be allowed, got %T: %v", err, err)
	}
