...
func TestGet_EffectiveWriteAccess(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "admin without project write access", directRole: "viewer", admin: true},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			project := createReadyProject(t, svc, db, "permissions", owner)
			if tt.directRole == "owner" {
				user = owner
			} else if tt.directRole != "" {
				if err := svc.rbac.GrantProjectAccess(user, project.ID, tt.directRole); err != nil {
					t.Fatal(err)
				}
			}
			if tt.groupRole != "" {
				...
				if err := svc.rbac.GrantGroupProjectAccess(group, project.ID, tt.groupRole); err != nil {
					t.Fatal(err)
				}
			}
			if tt.admin {
			...
			resp, err := svc.Get(project.ID.String(), user)
			if err != nil {
				t.Fatal(err)
			}
...
func (failingWriteAccessProvider) CanWriteProject(uuid.UUID, uuid.UUID) (bool, error) {
...
func TestGet_WriteAccessError(t *testing.T) {
	...
	resp, err := svc.Get(project.ID.String(), owner)
	if err == nil || resp != nil {
