...
// PublishProject publishes a project's pixi.toml and pixi.lock to an OCI registry.
func (s *ProjectService) PublishProject(ctx context.Context, projectID string, req PublishProjectRequest, userID uuid.UUID) (*PublicationResult, error) {
	...
	if project.Status != models.ProjectStatusReady {
		return nil, &ValidationError{Message: "Project must be in ready state to publish"}
	}

	// Get the latest version
	var latestVersion models.ProjectVersion
	if err := s.db.Where("project_id = ?", projectID).Order("version_number DESC").First(&latestVersion).Error; err != nil {
		return nil, &ValidationError{Message: "Project has no versions to publish"}
	}
	...
	s.db.Where("project_id = ? AND version_number = ?", project.ID, latestVersion.VersionNumber).Find(&projectTags)
	...
	if s.isLocal {
		...
		res, err := oci.Publish(ctx, projectPath, regEndpoint, req.Repository, req.Tag,
		...
		d, err := oci.PublishProject(ctx, projectPath, oci.PublishOptions{
			Repository:   fullRepo,
			Tag:          req.Tag,
			ExtraTags:    extraTags,
			Username:     ep.Username,
	...
	audit.Log(s.db, userID, audit.ActionPublishProject, audit.ResourceProject, project.ID, map[string]interface{}{
...
// ListPublications returns publications for a project that the user can read.
func (s *ProjectService) ListPublications(projectID string, userID uuid.UUID) ([]PublicationResult, error) {
	var project models.Project
	if err := s.db.Where("id = ?", projectID).First(&project).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, ErrNotFound
...
func (s *ProjectService) UpdatePublication(ctx context.Context, projectID string, pubID string, isPublic bool, userID uuid.UUID) (*PublicationResult, error) {
	var project models.Project
	if err := s.db.Where("id = ?", projectID).First(&project).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, ErrNotFound
	...
	if err := s.db.Where("id = ? AND project_id = ?", pubID, projectID).First(&publication).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, ErrNotFound
		}
		return nil, err
...
func (s *ProjectService) GetPublishDefaults(projectID string, userID uuid.UUID, registryID uuid.UUID) (*PublishDefaultsResult, error) {
	var project models.Project
	if err := s.db.Where("id = ?", projectID).First(&project).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, ErrNotFound
	...
	tag := "latest"
	var latestVersion models.ProjectVersion
	hasVersion := s.db.Where("project_id = ?", projectID).Order("version_number DESC").First(&latestVersion).Error == nil

	if hasVersion {
		if s.isLocal {
			projectPath := s.executor.GetProjectPath(&project)
			pixiToml, tomlErr := os.ReadFile(filepath.Join(projectPath, "pixi.toml"))
			pixiLock, lockErr := os.ReadFile(filepath.Join(projectPath, "pixi.lock"))
			// A project that has been pushed to but never installed yet
