...
// and returns a ProjectService ready for testing.
func testSetup(t *testing.T, isLocal bool) (*ProjectService, *gorm.DB) {
	t.Helper()
...
func createReadyProject(t *testing.T, svc *ProjectService, db *gorm.DB, name string, userID uuid.UUID) *models.Project {
	t.Helper()
	project, err := svc.Create(context.Background(), CreateRequest{Name: name}, userID)
	if err != nil {
		t.Fatalf("create project: %v", err)
	}
	db.Model(project).Update("status", models.ProjectStatusReady)
...
func TestCreate_Defaults(t *testing.T) {
	...
	project, err := svc.Create(context.Background(), CreateRequest{Name: "test-ws"}, userID)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if project.Status != models.ProjectStatusPending {
		t.Errorf("expected status=pending, got %q", project.Status)
	}
}
...
func TestCreate_LocalSourceAccepted(t *testing.T) {
	...
	project, err := svc.Create(context.Background(), CreateRequest{
		Name:   "local-ws",
		Source: "local",
		Path:   "/tmp/my-project",
	}, userID)
	...
	if project.Source != "local" || project.Path != "/tmp/my-project" {
		t.Errorf("unexpected source=%q path=%q", project.Source, project.Path)
	}
}
...
func TestCreate_RejectsOversizedPixiTomlBeforeWrites(t *testing.T) {
	...
	var projects, jobs int64
	db.Model(&models.Project{}).Count(&projects)
	db.Model(&models.Job{}).Count(&jobs)
	if projects != 0 || jobs != 0 {
		t.Fatalf("expected no project/job writes, got projects=%d jobs=%d", projects, jobs)
	}
}
...
func TestCreate_AllowsManifestWithManyPackages(t *testing.T) {
	...
	db.Model(&models.Project{}).Count(&projects)
	if projects != 1 {
		t.Fatalf("expected 1 project, got %d", projects)
	}
}
...
func TestList_LocalModeReturnsAll(t *testing.T) {
	...
	// In local mode, any user sees all projects
	projects, err := svc.List(alice)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(projects) != 2 {
		t.Errorf("expected 2 projects in local mode, got %d", len(projects))
	}
}
...
func TestList_TeamModeFiltersToOwner(t *testing.T) {
	...
	if len(projects) != 1 {
		t.Errorf("expected 1 project for alice in team mode, got %d", len(projects))
	}
	if len(projects) > 0 && projects[0].Name != "ws-alice" {
		t.Errorf("expected ws-alice, got %q", projects[0].Name)
	}
}
...
func TestGet_Found(t *testing.T) {
	...
	if project.Name != "test" {
		t.Errorf("expected name=test, got %q", project.Name)
	}
}
...
func TestDelete_BypassesActiveJobQuotas(t *testing.T) {
	...
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "delete-quota", userID)
	...
	if err := svc.Delete(context.Background(), project.ID.String(), userID); err != nil {
		t.Fatalf("delete should bypass active job quotas: %v", err)
	}

	var deleteJobs int64
	db.Model(&models.Job{}).
		Where("project_id = ? AND type = ? AND status = ?", project.ID, models.JobTypeDelete, models.JobStatusPending).
		Count(&deleteJobs)
	if deleteJobs != 1 {
...
func TestPushVersion_TagConflictWithoutForce(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "push-test", userID)

	// First push succeeds
	_, err := svc.PushVersion(context.Background(), project.ID.String(), PushRequest{
		Tag:      "v1",
		PixiToml: "[project]\nname = \"test\"",
	}, userID)
	if err != nil {
	...
	var count int64
	db.Model(&models.ProjectVersion{}).Where("project_id = ?", project.ID).Count(&count)
	if count != 1 {
...
func TestPushVersion_TagConflictWithForce(t *testing.T) {
	...
	// First push
	r1, err := svc.PushVersion(context.Background(), project.ID.String(), PushRequest{
		Tag:      "v1",
		PixiToml: "[project]\nname = \"test\"",
	}, userID)
	if err != nil {
	...
	// Force push same tag
	r2, err := svc.PushVersion(context.Background(), project.ID.String(), PushRequest{
		Tag:      "v1",
		PixiToml: "[project]\nname = \"test-v2\"",
		Force:    true,
	}, userID)
	...
	db.Where("project_id = ? AND tag = ?", project.ID, "v1").First(&tag)
	if tag.VersionNumber != r2.VersionNumber {
		t.Errorf("tag should point to version %d, got %d", r2.VersionNumber, tag.VersionNumber)
	}
}

func TestPushVersion_ProjectNotReady(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")

	// Create project but don't mark it ready (stays pending)
	project, err := svc.Create(context.Background(), CreateRequest{Name: "pending-ws"}, userID)
	if err != nil {
...
func TestPushVersion_RejectsOversizedLockBeforeVersionWrite(t *testing.T) {
	...
	svc.limits = limitCfg
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "push-lock-limit", userID)
...
func TestPixiToml_RoundTrip(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "toml-test", userID)
	...
	content := "[project]\nname = \"my-project\""
	if err := svc.SavePixiToml(project.ID.String(), content); err != nil {
		t.Fatalf("save: %v", err)
	}

	got, err := svc.GetPixiToml(project.ID.String())
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	if got != content {
...
func TestSavePixiToml_RejectsOversizedBeforeWrite(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "toml-limit", userID)
	limitCfg := limits.Defaults()
	limitCfg.ManifestBytes = 8
	svc.limits = limitCfg
	...
	manifestPath := filepath.Join(projectPath, "pixi.toml")
	original := "[project]\nname = \"toml-limit\"\n"
	if err := os.WriteFile(manifestPath, []byte(original), 0o644); err != nil {
		t.Fatalf("write original manifest: %v", err)
	}

	err := svc.SavePixiToml(project.ID.String(), strings.Repeat("x", 9))
...
func TestGetPixiToml_UsesPersistedPathForManagedProject(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "persisted-path", userID)
	...
	content := "[workspace]\nname = \"persisted-path\"\n"
	if err := os.WriteFile(filepath.Join(projectPath, "pixi.toml"), []byte(content), 0o644); err != nil {
		t.Fatalf("write pixi.toml: %v", err)
	}

	// Simulate worker-persisted path in DB.
	if err := db.Model(&models.Project{}).Where("id = ?", project.ID).Update("path", projectPath).Error; err != nil {
		t.Fatalf("persist path: %v", err)
	}

	// Simulate a later process with a different configured base dir.
	cfg := &config.Config{Storage: config.StorageConfig{ProjectsDir: t.TempDir()}}
	...
	got, err := svc2.GetPixiToml(project.ID.String())
	if err != nil {
...
func TestGetPixiToml_NotFoundProject(t *testing.T) {
	svc, _ := testSetup(t, true)
...
func TestGetPixiToml_MissingFile(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "no-toml", userID)

	// Project dir exists but no pixi.toml
	projectPath := svc.executor.GetProjectPath(project)
	os.MkdirAll(projectPath, 0755)

	_, err := svc.GetPixiToml(project.ID.String())
	if err != ErrNotFound {
...
func TestGetPixiToml_PermissionError(t *testing.T) {
	...
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "perm-test", userID)

	// Create file then remove read permission
	projectPath := svc.executor.GetProjectPath(project)
	os.MkdirAll(projectPath, 0755)
	pixiPath := filepath.Join(projectPath, "pixi.toml")
...
func TestListVersions_Empty(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "no-versions", userID)

	versions, err := svc.ListVersions(project.ID.String())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(versions) != 0 {
...
func TestListVersions_IncludesManifestVersion(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	ws := createReadyProject(t, svc, db, "versioned", userID)

	if err := db.Create(&models.ProjectVersion{
		ProjectID:       ws.ID,
		VersionNumber:   1,
...
func TestGetVersion_IncludesManifestVersion(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	ws := createReadyProject(t, svc, db, "single-versioned", userID)
...
func TestListTags_Empty(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "no-tags", userID)

	tags, err := svc.ListTags(project.ID.String())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(tags) != 0 {
...
func TestListTags_AfterPush(t *testing.T) {
	svc, db := testSetup(t, true)
	userID := createTestUser(t, db, "alice")
	project := createReadyProject(t, svc, db, "tag-test", userID)

	if _, err := svc.PushVersion(context.Background(), project.ID.String(), PushRequest{
		Tag:      "latest",
		PixiToml: "[project]\nname = \"tag-test\"\n",
	}, userID); err != nil {
...
func TestCreate_ForwardsImportStagingDirToJob(t *testing.T) {
	...
	var job models.Job
	if err := db.Where("project_id = ? AND type = ?", project.ID, models.JobTypeCreate).First(&job).Error; err != nil {
...
func TestProjectService_IsLocal(t *testing.T) {
