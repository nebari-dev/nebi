...
// makeProjectWithDir creates a tracked project pointing at a fresh
// temp directory and returns it.
func makeProjectWithDir(t *testing.T, s *Store, name string) *LocalProject {
	t.Helper()
	dir := t.TempDir()
	project := &LocalProject{Name: name, Path: dir}
...
func TestCreateVersion_DedupAgainstLatest(t *testing.T) {
	s := testStore(t)
	project := makeProjectWithDir(t, s, "dedup-test")

	manifest := "[workspace]\nname = \"dedup-test\"\n"
	lock := "version: 6\n"

	v1, created, err := s.CreateVersion(project.ID, manifest, lock, "first")
	if err != nil {
	...
	v2, created, err := s.CreateVersion(project.ID, manifest, lock, "second attempt")
	if err != nil {
	...
	v3, created, err := s.CreateVersion(project.ID, manifest+"\n# tweak\n", lock, "third")
	if err != nil {
...
func TestListVersions(t *testing.T) {
	s := testStore(t)
	project := makeProjectWithDir(t, s, "list-test")
	...
	for i := 1; i <= 3; i++ {
		manifest := "[workspace]\nv = " + string(rune('0'+i)) + "\n"
		if _, _, err := s.CreateVersion(project.ID, manifest, "", "v"+string(rune('0'+i))); err != nil {
...
func TestGetVersion(t *testing.T) {
	s := testStore(t)
	project := makeProjectWithDir(t, s, "get-test")

	manifest := "[workspace]\nname = \"get-test\"\n"
	lock := "version: 6\nfoo: bar\n"
	if _, _, err := s.CreateVersion(project.ID, manifest, lock, "first"); err != nil {
		t.Fatal(err)
	}

	got, err := s.GetVersion(project.ID, 1)
	if err != nil {
	...
	missing, err := s.GetVersion(project.ID, 99)
	if err != nil {
...
func TestRollbackToVersion(t *testing.T) {
	s := testStore(t)
	project := makeProjectWithDir(t, s, "rollback-test")

	v1Manifest := "[workspace]\nname = \"rollback-test\"\nversion = \"1\"\n"
	v1Lock := "version: 6\n# v1\n"
	if _, _, err := s.CreateVersion(project.ID, v1Manifest, v1Lock, "v1"); err != nil {
		t.Fatal(err)
	}

	v2Manifest := "[workspace]\nname = \"rollback-test\"\nversion = \"2\"\n"
	v2Lock := "version: 6\n# v2\n"
	if _, _, err := s.CreateVersion(project.ID, v2Manifest, v2Lock, "v2"); err != nil {
		t.Fatal(err)
	}

	// Simulate a "current" disk state matching v2.
	if err := os.WriteFile(filepath.Join(project.Path, "pixi.toml"), []byte(v2Manifest), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project.Path, "pixi.lock"), []byte(v2Lock), 0644); err != nil {
		t.Fatal(err)
	}

	// Roll back to v1.
	v3, err := s.RollbackToVersion(project.ID, 1)
	if err != nil {
	...
	gotManifest, err := os.ReadFile(filepath.Join(project.Path, "pixi.toml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	gotLock, err := os.ReadFile(filepath.Join(project.Path, "pixi.lock"))
	if err != nil {
		t.Fatal(err)
	}
	...
	stored, err := s.GetVersion(project.ID, 3)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRollbackToVersion_NotFound(t *testing.T) {
	s := testStore(t)
	project := makeProjectWithDir(t, s, "rb-missing")

	if _, err := s.RollbackToVersion(project.ID, 42); err == nil {
