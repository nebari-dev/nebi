...
func TestStartCancellationPersistsFailureAndCleansUp(t *testing.T) {
	...
	_, job := newTestProject(t, db, exec, "cancel-install", models.JobTypeInstall,
		map[string]interface{}{"packages": []string{"numpy"}})
	q := queue.NewMemoryQueue(10)
	defer q.Close()
	...
	pending := &models.Job{ProjectID: job.ProjectID, Type: models.JobTypeUpdate, Status: models.JobStatusPending}
	...
	w := New(q, exec, svc, jobSvc, slog.Default(), limits.Defaults(), 1)
